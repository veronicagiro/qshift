...
As an alternative approach, we decided to develop package `qdist` available at [https://github.com/andreaspano/qdist](https://github.com/andreaspano/qdist) .
...
The newly generated function `tpnorm()` has the same formals as the original `pnorm()`, plus two
...
Once we have defined `tpnorm()` we can use it for generating probability values from a non-
...
df <- data.frame(
  q=q,
  tpnorm_type = factor(c(rep("p_L_U", times=length(q)), rep("p_L9_U11", times=length(q))), levels = c("p_L_U", "p_L9_U11")),
...
first generate function `tdweibull()` by using `dtruncate()`:
...
df <- data.frame(
  x=x,
  tdweibull_type = factor(c(rep("d_L0", times=length(x)), rep("d_L3", times=length(x)), rep("d_L5", times=length(x)), rep("d_L7", times=length(x))), levels = c("d_L0", "d_L3", "d_L5", "d_L7")),
...
```{r}
df <- data.frame(p=p,
  tqpois_type=factor(c(rep("No truncation", times=length(p)), rep("Lower truncation L = 9", times=length(p)), 
