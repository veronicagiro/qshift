...
We have to define also the others functions for truncated distribution, as they are necessary implicitly for the further step:
...
We can conclude that the previous functions are completely different.
...
pl <- ggplot(data = df, mapping = aes(x=x, y=value, col=type)) +
  geom_line(size=1) +
  scale_color_manual(values = c("std_rand" = "cyan3", 
                                "st_rand" = "#89FF99", 
                                "ts_rand" = "#FF89B4")) +
  xlab("Value") + ylab("Density") +
  ggtitle("Distribution plot for transformed and not-transformed Normal distributions") +
...
pensare ad un modo diverso per spiegare la cosa perchè da questi grafici nn si capisce niente
-->
