...
For more details and examples about `qdist` and `qshift` see articles "R package qdist: A functional programming approach to truncated probability ditributions" and "R package qshift: A functional programming approach to shifted probability distributions" available on [Quantide blog](http://www.quantide.com/blog/).

# Distribution function for truncated and shifted or for shifted and truncated distributions

In the previous paragraph we saw how `qdist` and `qshift` packages works and that they are built with a functional programming approach. The functional programming approach whereby the packages are built is the key that allows to obtain distribution functions for truncated and shifted or for shifted and truncated distributions. Let us see how!

Suppose we want to generate the probability functions for a truncated shifted normal distribution.  
As `pnorm()` is the function that computes the distribution function for a normal distributions, conventionally named `norm`, `ptnorm()`, defined as: 
```{r}
...
So `pstnorm()` is the function that computes the distribution function of `stnorm`, the shifted truncated normal distribution.
...
The inverse procedure are necessary to generate the distribution function for a shifted truncated distribution.
...
`psnorm()` is the distribution function of `snorm`, a shifted normal distribution.
The distribution function of a truncated shifted normal distribution is generated passing `snorm` distribution as `ptruncate()` argument:
