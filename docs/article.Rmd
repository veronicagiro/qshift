---
title: 'R package qtdshift: A functional programming approach to shifted probability
  ditributions'
author: "Veronica"
date: "August 25, 2016"
...
This paper aims to be an implementation of the methodology used by Andrea Spanò article, "R package qdist: A functional programming approach to truncated probability ditributions", which provides an alternative method for computing from truncated probability distribution using a functional programming approach. I was inspired by his work to implement the same methodology to shifted probability distributions. 

## Shifted probability functions in R

Why shifted distributions? Shift is a distribution trasformation used daily in many statistical fields. R does not provide a specific package for shifted distribution models, but it provides package `stats` which contains a wide set of functions for standard distribution models. I refer to: probability, density, quantile and random number generator functions, that share a stable naming convention both for the names of the functions and in the first argument of these functions. In particular:
...
Spano S. .  _R package qdist: A functional programming approach to truncated probability ditributions_. url
